...
import { useRegisterVendor } from "../hooks/useUser";

const VendorSignup = () => {
  const navigate = useNavigate();
  const { mutate: registerVendor, isPending } = useRegisterVendor();
  ...
  const validate = () => {
    ...
    if (!formData.businessName?.trim())
      e.businessName = "Business name required";
    if (!formData.businessCategory) e.businessCategory = "Category required";
    if (formData.businessCategory === "Others" && !formData.specificCategory)
      e.specificCategory = "Specific category required";
    if (!formData.shopImage) e.shopImage = "Shop image required";
  ...
  const handleCancelCategory = () => {
    setShowCategoryModal(false);
    setPendingCategory("");
    const sel = document.querySelector('select[name="businessCategory"]');
    if (sel) sel.value = formData.businessCategory;
  };
  ...
  const handleSubmit = (e) => {
    ...
    fd.append("user_type", "vendor");
    fd.append("business_name", formData.businessName);
    fd.append("business_category", formData.businessCategory.toLowerCase());
    if (formData.businessCategory === "Others")
      fd.append("specific_category", formData.specificCategory);
    ...
    if (formData.shopImage) fd.append("shop_image", formData.shopImage);
  ...
  const inputClass =
    "w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200";
        ...
        >
          ...
          <div className="text-center pt-8 pb-6 px-6 bg-gradient-to-b from-white to-gray-50 relative">
            ...
            <p className="text-gray-500 text-sm mt-1">
              Sell your products to students nearby
            </p>
          </div>
          ...
          <div className="px-6 pb-8 space-y-5">
            ...
            <div>
              <label className={labelClass}>
                <FaHouseUser className="mr-2 text-emerald-500" />
                Residence
              </label>
              <select
                name="residence"
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                accept="image/*"
                className={`${inputClass} file:mr-4 file:py-1.5 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700`}
              />
              {errors.profilePic && (
            ...
            <div className="border-t border-gray-100 pt-5">
              <h3 className="text-base font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <FaBuilding className="text-emerald-500" /> Business Details
              </h3>
              <div className="space-y-4">
                ...
                <div>
                  <label className={labelClass}>
                    <FaList className="mr-2 text-emerald-500" />
                    Business Category
                  </label>
                  <select
                    name="businessCategory"
            ...
            <div className="border-t border-gray-100 pt-5">
              <h3 className="text-base font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <FaMoneyBill className="text-emerald-500" /> Bank Account
                Details
              </h3>
              <div className="space-y-4">
                <div>
                  <label className={labelClass}>
                    <FaMoneyBill className="mr-2 text-emerald-500" />
                    Bank Name
                  </label>
                  <select
                    name="bankName"
            ...
            <button
              ...
              style={{
                backgroundColor:
                  isPending || throttleWaitTime > 0 ? "#d1d5db" : "#10b981",
